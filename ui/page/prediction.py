...
experience_lov = ['1年以内', '1-3年', '3-5年',
                  '5-10年', '10年以上', '在校/应届', '应届生', '经验不限']
...
def btn_predict(state):
    if not (state.category_input == '' or state.sub_category_input == '' or state.province_input == ''):
        state.predicted_salary = predict_salary(
            model, state.category_input, state.sub_category_input,
            state.province_input, state.experience_input
        )
    else:
